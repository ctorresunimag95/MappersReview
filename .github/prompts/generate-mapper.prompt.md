---
agent: 'agent'
description: 'Generate a manual mapper (extension method or DI-registered) between two classes.'
---

# Generate a manual mapper

Generate a manual mapper between two classes.

## Inputs

- Source class: `${input:source:SourceClass (e.g. Order)}`
- Destination class: `${input:destination:DestinationClass (e.g. OrderDto)}`
- Style: `${input:style:extension or di — leave blank to be asked}`
- Work location: `${input:location:target directory/file — leave blank to infer}`

Rules for the inputs:
- If the **source** or **destination** class is missing, ask me for it before generating anything.
- The **style** selects the mapper shape: `extension` (an extension method) or `di` (a DI-registered
  mapper). **If the style is blank, ask me** which one I want before generating anything.
- **Ask me for a preferred work location** if it is blank. If I have no preference, infer a sensible
  location from where existing mappers/models live in the codebase.
- If a destination member has multiple plausible source members, **ask me instead of guessing**.
- If a nested source path may be null and the destination member is non-nullable, do not silently
  invent a fallback. Use the project's existing null-handling pattern when one exists; otherwise ask.
- Only apply trivial safe conversions automatically (e.g. exact type match, nullable-to-nullable, or
  obvious string passthrough). For non-trivial conversions, formatting, enums, or lossy casts, ask me
  or clearly report the member as skipped.

## Steps

1. **Find the source class**: Search the codebase for a class named `${input:source}`. Read its full
   definition including all properties and their types. Also read any nested types it references.

2. **Find or create the destination class**: Search for `${input:destination}`. If it exists, read it.
   If it does not exist, infer reasonable property names and types from the source and create it as a
   new file, matching the existing model/POCO style in the codebase.
...
4. **Generate the mapper** in the chosen work location, named `<SourceClass>Mapper.cs`.
   ...
   reported, or blocked pending clarification. Do not leave ambiguous members unmentioned.
...
5. **Register the DI mapper** (only when style is `di`): add the registration to the application's
   ...
   matching the surrounding registration style. If there is no obvious place to register it, ask me
   where the mapper should be registered.
